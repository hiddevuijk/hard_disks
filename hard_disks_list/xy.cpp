...
double xy::dist(const XY &c1, const XY &c2)
{ return sqrt( (c1.x - c2.x)*(c1.x - c2.x) +
			(c1.y - c2.y)*(c1.y - c2.y) ); }

// same as dist2, but with periodic boundary conditions
double xy::dist_pbc(const XY &c1, const XY &c2, double L)
{
	XY d;
	d.x = c1.x - c2.x;
	d.y = c1.y - c2.y;
...
double xy::dist_sq(const XY &c1, const XY &c2)
{ return  (c1.x - c2.x)*(c1.x - c2.x) +
			(c1.y - c2.y)*(c1.y - c2.y);
}

// same as dist2_sq, but with periodic boundary conditions
double xy::dist_sq_pbc(const XY &c1, const XY &c2, double L)
{
	XY d;
	d.x = c1.x - c2.x;
	d.y = c1.y - c2.y; 
